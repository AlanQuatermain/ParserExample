...
@implementation ParserExampleAppDelegate
...
- (void) fetchData
{
    ...
    // setup the request message -- it's pretty basic for Tumblr
    NSURL * url = [NSURL URLWithString: @"http://quatermain.tumblr.com/api/read"];
    HTTPMessage * msg = [HTTPMessage requestMessageWithMethod: @"POST"
														  url: url
													  version: HTTPVersion1_1];
    
    // Looks like Tumblr never gzips its replies...?
    ...
    // build the parser around the gzip stream
    parser = [[AQXMLParserWithTimeout alloc] initWithStream: bareStream];   // use gzipStream if using gzip encoding
    //[gzipStream release];
    parser.timeout = 30.0;
    
    // set a delegate for the parser
                                         ...
                                         context: NULL] == NO )
    {
        ...
        UIAlertView * alert = [[UIAlertView alloc] initWithTitle: @"XML Parser Error" message: [NSString stringWithFormat: @"%@ (%@)", [error localizedDescription], [error localizedFailureReason]] delegate: nil cancelButtonTitle: @"Argh!" otherButtonTitles:nil];
        [alert show];
        [alert release];
...
- (NSManagedObjectModel *)managedObjectModel {
    ...
    NSString *modelPath = [[NSBundle mainBundle] pathForResource:@"ParserExample" ofType:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"ParserExample.sqlite"];
